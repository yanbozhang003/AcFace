function fs_final = z_get_spec_final(rx_trace_list)
    %% get experiment settings and configuration parameters
...
%                     fprintf('tr_idx: %d | rx_idx: %d | mic_idx: %d\n', rx_trace_idx, rx_idx, mic_idx);
                    pc = ((rx_idx-1)*NUM_MIC+mic_idx)/(NUM_MIC*NUM_MIC_ARRAY);
                    fprintf('\rProcessing complete: %.2f%%', pc*100);
...
%                     plot(I_to_test,DC_volt_vec)
    ...
    speaker_position = setting_params.speaker_positon;
    
    faceArrayDist = config.distance_cm;
    ...
    signal_resolution = acoustic_speed / signal_bandwidth;
    detect_range = proc_params.fmcw_range;
        ...
        for z_i = 1:num_z
            for x_i = 1:num_x
                ...
                for i_temp = 1:length(I_v)
                    current_log = cir_mat_log{micIdx_nearest(i_temp), I_v(i_temp), y_i};
                    if isempty(current_log)
                        current_log.list = cube_counter;
                        cir_mat_log_length(micIdx_nearest(i_temp), I_v(i_temp), y_i) = 1;
                    else
                        current_log.list(end + 1) = cube_counter;
                        cir_mat_log_length(micIdx_nearest(i_temp), I_v(i_temp), y_i) = ...
                            cir_mat_log_length(micIdx_nearest(i_temp), I_v(i_temp), y_i) + 1;
                    end    
                    cir_mat_log{micIdx_nearest(i_temp), I_v(i_temp), y_i} = current_log;
                end
            end
        end

        cube_fill_record = zeros(1,2);
        cube_filled_count = 0;
        ...
        while true
            ...
            for i_tmp = 1:length(rowIndices)
                ...
                for cube_i = 1:length(cubes_projected)
                    ...
                    [cube_power, power_vec] = get_cube_fed(cir_mat_new, facial_spec,speaker_position, ...
                                            mic_position_vec, cir_ref, config, cube_indice, x_vec, y_vec, z_vec);
                    ...
                    for assoc_cir_grid_i = 1:num_project_cube
                        rowIdx = cube_all_assoc_cir_grids_indices(assoc_cir_grid_i, 1);
                        colIdx = cube_all_assoc_cir_grids_indices(assoc_cir_grid_i, 2);
                        cir_mat(rowIdx, colIdx) = cir_mat(rowIdx, colIdx) - single_project_power;
                        cir_mat_log_length_i(rowIdx, colIdx) = cir_mat_log_length_i(rowIdx, colIdx) - 1;
                    end
                end
            end
        
            if all(cir_mat_log_length_i(:) == 0)
                break
            end
        end
...
function ret_indice = get_assoc_indice(cube_indice, cir_log)
    ...
    ret_indice = zeros(10, 2);
    indice_count = 0;
    
    for row_i = 1:num_row
        for col_i = 1:num_col
            if isempty(cir_log{row_i, col_i}) || isempty(cir_log{row_i, col_i}.list)
                continue;
            end
            ...
            if ismember(cube_indice, cube_indice_list)
                ...
                if indice_count > size(ret_indice, 1)
                    ret_indice = [ret_indice; zeros(size(ret_indice, 1), 2)];
                end
                ret_indice(indice_count, :) = [row_i, col_i];
            end
        end
    end
    
    ret_indice = ret_indice(1:indice_count, :);
end

function [cube_power, power_vec] = get_cube_fed(cir_mat_new, facial_spec, speaker_position, ...
                                    mic_position_vec, cir_ref, config, cube_indice, x_vec, y_vec, z_vec)
